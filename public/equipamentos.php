...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($_POST['create_equipamento'])) {
        ...
        $numero_tombamento = isset($_POST['numero_tombamento']) ? mysqli_real_escape_string($conn, $_POST['numero_tombamento']) : '';
        ...
        // Insert (adicionando created_att e updated_at com NOW())
        $sql = "INSERT INTO equipamentos (nome, tipo, fk_ala, descricao, imagem, numero_tombamento, status, created_at, updated_at) 
                VALUES (?, ?, ?, ?, ?, ?, 'ativo', NOW(), NOW())";
        $stmt = $conn->prepare($sql);
        if ($stmt) {
            // tipos: nome(s), tipo(s), fk_ala(i), descricao(s), imagem(s), numero_tombamento(s)
            $stmt->bind_param("ssisss", $nome, $tipo, $fk_ala, $descricao, $imagem, $numero_tombamento);

            if ($stmt->execute()) {
                $message = 'Equipamento criado com sucesso!';
                $action = 'list';
            } else {
                $error = 'Erro ao criar equipamento: ' . $stmt->error;
            }
            $stmt->close();
        } else {
            $error = 'Erro no prepare do INSERT: ' . $conn->error;
        }
    ...
    // ---------- UPDATE ----------
        ...
        $stmt_img = $conn->prepare($current_image_sql);
        if ($stmt_img) {
            $stmt_img->bind_param("i", $id);
            $stmt_img->execute();
            $res_img = $stmt_img->get_result();
            $current_data = $res_img->fetch_assoc();
            $imagem = $current_data ? $current_data['imagem'] : '';
            $stmt_img->close();
        } else {
            // se falhar, seta imagem vazia para não quebrar
            $imagem = '';
        }

        // Handle new image upload (se enviado, substitui e remove antigo)
        if (isset($_FILES['imagem']) && $_FILES['imagem']['error'] == 0) {
            $upload_dir = '../uploads/equipamentos/';
            if (!file_exists($upload_dir)) {
                mkdir($upload_dir, 0777, true);
        ...
        // UPDATE com numero_tombamento e updated_at
        $sql = "UPDATE equipamentos 
                SET nome = ?, tipo = ?, fk_ala = ?, descricao = ?, imagem = ?, numero_tombamento = ?, status = ?, updated_at = NOW()
                WHERE id = ?";
        $stmt = $conn->prepare($sql);
        if ($stmt) {
            // tipos: nome(s), tipo(s), fk_ala(i), descricao(s), imagem(s), numero_tombamento(s), status(s), id(i)
            $stmt->bind_param("ssissssi", $nome, $tipo, $fk_ala, $descricao, $imagem, $numero_tombamento, $status, $id);

            if ($stmt->execute()) {
                $message = 'Equipamento atualizado com sucesso!';
                $action = 'list';
            } else {
                $error = 'Erro ao atualizar equipamento: ' . $stmt->error;
            }
            $stmt->close();
        } else {
            $error = 'Erro no prepare do UPDATE: ' . $conn->error;
        }
    ...
    // ---------- DELETE (soft) ----------
        ...
        // Soft delete
        $sql = "UPDATE equipamentos SET status = 'inativo', updated_at = NOW() WHERE id = ?";
        $stmt = $conn->prepare($sql);
        if ($stmt) {
            ...
            if ($stmt->execute()) {
                ...
                $error = 'Erro ao desativar equipamento: ' . $stmt->error;
            }
            $stmt->close();
        } else {
            $error = 'Erro no prepare do DELETE: ' . $conn->error;
        }
    }
}
...
if (($action == 'edit' || $action == 'view') && $equip_id > 0) {
    ...
    if ($stmt) {
        ...
        $error = 'Erro ao buscar equipamento: ' . $conn->error;
    }
}

// Get all equipment (só ativos)
            ...
            <div class="content-wrapper">
                ...
                <?php if ($action == 'list'): ?>
                    ...
                    <div class="card">
                        ...
                        <div class="card-body">
                            <?php if ($equipamentos_result && $equipamentos_result->num_rows > 0): ?>
                                        ...
                                        <tbody>
                                            <?php while ($equip = $equipamentos_result->fetch_assoc()): ?>
                                                <tr>
                                                    ...
                                                    <td>
                                                        <a href="?action=view&id=<?php echo $equip['id']; ?>" class="btn btn-sm btn-info" title="Ver">
                                                            <i class="bi bi-eye"></i>
                                                        </a>
                                                        <a href="?action=edit&id=<?php echo $equip['id']; ?>" class="btn btn-sm btn-warning" title="Editar">
                                                        ...
                                                        <form method="POST" style="display:inline;" onsubmit="return confirmDelete('Deseja realmente desativar este equipamento?');">
                                                            ...
                                                            <button type="submit" name="delete_equipamento" class="btn btn-sm btn-danger" title="Desativar">
                    ...
                    <div class="card">
                        ...
                        <div class="card-body">
                            <form method="POST" enctype="multipart/form-data" class="needs-validation" novalidate>
                                ...
                                <div class="row">
                                    <div class="col-md-6 mb-3">
                                        <label for="numero_tombamento" class="form-label">Número de Tombamento</label>
                                        <input type="text" class="form-control" id="numero_tombamento" name="numero_tombamento" maxlength="50">
                                    </div>
                                </div>
                        ...
                        <div class="card-body">
                            <form method="POST" enctype="multipart/form-data" class="needs-validation" novalidate>
                                ...
                                <div class="row">
                                    <div class="col-md-6 mb-3">
                                        <label for="numero_tombamento" class="form-label">Número de Tombamento</label>
                                        <input type="text" class="form-control" id="numero_tombamento" name="numero_tombamento" maxlength="50" value="<?php echo htmlspecialchars($equip_data['numero_tombamento']); ?>">
                                    </div>
                                </div>
                        ...
                        <div class="card-body">
                            <div class="row">
                                ...
                                <div class="col-md-8">
                                    ...
                                    <p><strong>Tombamento:</strong> <?php echo htmlspecialchars($equip_data['numero_tombamento']); ?></p>
